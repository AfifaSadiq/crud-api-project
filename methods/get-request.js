module.exports = (req,res) => {
    ...
    if(req.url === "/api/movies"){
        ...
        res.end();
    }else if(!regexV4.test(id)){//wrong id, return response error msg
        res.writeHead(400, {"Content-Type":"application/json"});
        res.end(JSON.stringify({title : "Validation Failed", message: "UUID is not valid"}));
    }else if(baseUrl === "/api/movies/" && regexV4.test(id)){//correct id, return movie
...
//for getting all movies-http://5000/api/movies